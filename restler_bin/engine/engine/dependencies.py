...
# SSTG instrumentation: structured NDJSON trace of dynamic object assignments.
# Enabled by setting environment variable SSTG_TRACE_DYNOBJ=1 before starting RESTler.
_SSTG_TRACE_ENABLED = os.environ.get('SSTG_TRACE_DYNOBJ', '0') == '1'
_SSTG_TRACE_FILE = None
_SSTG_TRACE_LOCK = threading.Lock()

def _sstg_trace_init(log_dir=None):
    """Initialize the SSTG dynamic object trace file.
    Called lazily on the first set_variable event when tracing is enabled.
    """
    global _SSTG_TRACE_FILE
    if _SSTG_TRACE_FILE is not None:
        return
    if log_dir is None:
        try:
            ...
            log_dir = '.'
    trace_path = os.path.join(log_dir, 'sstg_dynobj_trace.ndjson')
    _SSTG_TRACE_FILE = open(trace_path, 'a', encoding='utf-8')
...
def set_variable(type, value):
    ...
    if _SSTG_TRACE_ENABLED:
        thread_id = threading.current_thread().ident
        # Safe preview: first 80 chars of value, avoiding log injection
        val_preview = str(value)[:80] if value is not None else 'None'
        record = {
            ...
            'counter': object_creations,
            'thread_id': thread_id,
        }
        with _SSTG_TRACE_LOCK:
            _sstg_trace_init()
            _SSTG_TRACE_FILE.write(json.dumps(record) + '\n')
            _SSTG_TRACE_FILE.flush()
